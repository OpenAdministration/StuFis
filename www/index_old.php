...
function writeFormdata($antrag_id, $isPartiell, $form, $antrag){
    ...
    if (isset($_REQUEST["projektposten"])){
        $pp = $_REQUEST["projektposten"];
        $formtype = $_REQUEST["formtype"];
        //check that all properties have same amount of entrys
        $rowCount = count($pp["geld.name"]);
        $ret &= ($rowCount === count($pp["geld.bemerkung"]));
        $ret &= ($rowCount === count($pp["geld.einnahmen"]));
        $ret &= ($rowCount === count($pp["geld.ausgaben"]));
        if (isset($pp["geld.titel"]))
            $ret &= ($rowCount === count($pp["geld.titel"]));
        
        //only iterate over n-1 rows, last one is auto-generated prefill
        for ($i = 0; $i < $rowCount - 1; $i++){
            $inhalt = [];
            $inhalt["id"] = $i;
            $inhalt["projekt_id"] = $antrag_id;
            $inhalt["name"] = $pp["geld.name"][$i];
            $inhalt["bemerkung"] = $pp["geld.bemerkung"][$i];
            $inhalt["einnahmen"] = convertUserValueToDBValue($pp["geld.einnahmen"][$i], $formtype["geld.einnahmen"]);
            $inhalt["ausgaben"] = convertUserValueToDBValue($pp["geld.ausgaben"][$i], $formtype["geld.ausgaben"]);
            if (isset($pp["geld.titel"]))
                $inhalt["titel"] = convertUserValueToDBValue($pp["geld.titel"][$i], $formtype["geld.titel"]);
            //print_r($inhalt);
            $ret1 = false !== DBConnector::getInstance()->dbInsert("projektposten", $inhalt); //FIXME: lastId() returns true, but sql works :/
            //print_r($ret1);
            $ret &= $ret1;
        }
    }
    ...
    if (!isset($_REQUEST["nonce"]) || $_REQUEST["nonce"] !== $nonce){
        ...
        switch ($_POST["action"]):
            ...
            case "antrag.create":
                ...
                $antrag["stateCreator"] = AuthHandler::getInstance()->getUsername();
                $ret = DBConnector::getInstance()->dbInsert("antrag", $antrag);
                if ($ret !== false){
                    $target = str_replace("//", "/", $URIBASE . "/") . rawurlencode($token);
                    $antrag_id = (int)$ret;
                    #$msgs[] = "Antrag wurde erstellt.";
                ...
                if ($ret && $doImport){
                    foreach ($oldAntrag["_inhalt"] as $ih){
                        ...
                        if (isset($ih["id"]))
                            unset($ih["id"]);
                        $ret0 = DBConnector::getInstance()->dbInsert("inhalt", $ih);
                        if ($ret0 === false){
                ...
                if ($ret && $doImport){
                    foreach ($oldAntrag["_anhang"] as $ah){
                        ...
                        $ret0 = DBConnector::getInstance()->dbInsert("anhang", $ah);
                        if ($ret0 === false){
                            $msgs[] = "Failed to import file";
                            $ret = false;
                            break;
            ...
            case "hibiscus":
                ...
                $newZahlungen = HibiscusXMLRPCConnector::getInstance()->fetchFromHibiscus();
                $lastDate = false;
                foreach ($newZahlungen as $zahlung){
                    $fields = [];
                    ...
                    $fields['value'] = convertUserValueToDBValue($zahlung["betrag"], "money");
                ...
                if ($ret){
                    if ($lastDate === false)
                        $msgs[] = "Keine neuen/nicht bekannte Kontobewegungen gefunden";
                    else
                        $msgs[] = "Letzte neue Kontobewegung von $lastDate gefunden und heruntergeladen";
                    $forceClose = true;
                    $target = "$URIBASE?tab=konto";
                }
                break;
            case "booking":
                AuthHandler::getInstance()->requireGroup($HIBISCUSGROUP);
            ...
            */
            ...
            case "mykonto.update":
                ...
                if (DBConnector::getInstance()->getUser() !== false)
...
switch ($tabName){
    ...
    case "allgremium":
        ...
        Menu_Renderer::renderProjekte($gremien);
        break;
    
    case "mykonto":
        Menu_Renderer::renderMyProfile($nonce);
        break;
    case "stura":
        ...
        $mapping[] = ["p-name" => "projekt.name", "org-name" => "projekt.org.name"];
        $mapping[] = ["p-name" => "projekt.name", "org-name" => "projekt.org.name"];
        Menu_Renderer::renderTable($groups, $mapping);
        break;
    case "hv":
    ...
    case "hhp":
        ...
        Menu_Renderer::renderHaushaltsplan($antrag_id);
        break;
    case "antrag":
        $antrag = getAntrag();
        $form = getForm($antrag["type"], $antrag["revision"]);
    ...
    case "konto":
        ...
        Menu_Renderer::renderKonto($selected_hhp_id);
        break;
    case "hibiscus.sct":
        $tmp = DBConnector::getInstance()->dbFetchAll("antrag", [], ["id" => false], []);
        $antraege = [];
        foreach ($tmp as $t){
            $form = getForm($t["type"], $t["revision"]);
            if (false === $form) continue;
    ...
    case "booking.history":
        ...
        Menu_Renderer::renderBookingHistory($selected_hhp_id);
        //TODO FIXME;
        break;
    default:

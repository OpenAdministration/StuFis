...
$(document).ready(function () {
    ...
    $('.selectpicker').each(function (i, e) {
        var $e = $(e);
        if ($e.closest(".select-picker-container").length > 0) {
            return;
        }
        $e.selectpicker({
            style: 'btn-default',
    ...
    $(".select-picker-container").each(function (i, e) {
        $(e).triggerHandler("clone-post");
    });
    ...
    $(".single-file-container,.multi-file-container").each(function (i, e) {
        $(e).triggerHandler("clone-post");
    });
...
// FIXME not yet implemented in php for print/read-only version
    ...
    $("table.summing-table").on("update-summing-table.sum", function (evt) {
        ...
        $tfoot.children("tr").children(".cell-has-printSum").find("*[data-printSum]").each(function (i, e) {
            var $e = $(e);
            $e.data("print-sum-region", function ($out) {
                return $out.closest("table");
            });
            $e.triggerHandler("update-print-sum");
        });
    });
    ...
    $('.dynamic-table').each(function (i, table) {
        ...
        if (numIdCtr < numOldRows) {
            alert("invalid id ctr");
            numIdCtr = numOldRows;
        }
        ;
...
//      console.log("update "+fieldName+" to "+newVal);
    ...
    $(":input[data-extra-text]").on("change.extra-text", function (evt) {
        ...
        $(e).triggerHandler("change.extra-text");
    });
...
function onClickNewRow($tr, $table, tableId) {
    ...
    $ntr.appendTo($tbody);
    /* insert first so suffix can be found */
...
function updateInvRef($sel, newRef) {
    ...
    if (oldRefEmpty || (oldRef != newRef)) {
        ...
        $("table[orig-id]")
            .filter(function () {
                return $(this).attr("orig-id") == tableBaseName;
            })
            .children("tbody").children("tr").children(".store-row-id")
            .filter(function () {
                return $(this).val() == rowIdentifier;
            })
            .each(function (i, e) {
                var $e = $(e);
        ...
        var $table = $("table[name]").filter(function () {
            return extractFieldName($(this).attr("name")) == tableName;
        });
        if ($table.length != 1) {
            console.log("cannot find table instance referenced: " + tableName);
            return;
        }
        var $tr = $table.children("tbody").children("tr[dynamic-table-row-number]").filter(function () {
            return $(this).attr("dynamic-table-row-number") == rowNumber;
        });

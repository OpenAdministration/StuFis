        ...
        <div class="mt-8 flex flex-col overflow-hidden bg-white shadow sm:rounded-md">
            <ul role="list" class="divide-y divide-gray-200">
                @foreach($plans as $plan)
                    <li>
                    <a href="{{ route('budget-plan.show', ['id' => 1]) }}" class="block group hover:bg-gray-100">
                        <div class="px-4 py-4 sm:px-6">
                            <div class="flex items-center">
                                <p class="truncate text-sm font-medium text-indigo-600">{{ $plan->organisation }}</p>
                                <div class="ml-2 flex flex-shrink-0">
                                    <p class="inline-flex rounded-full bg-green-100 px-2 text-xs font-semibold leading-5 text-green-800">{{ $plan->state }}</p>
                                </div>
                            </div>
                            <div class="mt-2 sm:flex sm:justify-between">
                                <div class="sm:flex">
                                    <p class="flex items-center text-sm text-gray-500">
                                        ...
                                        <x-date :date="$plan->start_date" format="M y" />-<x-date :date="$plan->end_date" format="M y" />
                                    </p>

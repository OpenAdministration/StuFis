...
class AuslagenHandler2 extends FormHandlerInterface{
    ...
    private function getDbProject(){
        $res = $this->db->dbFetchAll("projekte", [DBConnector::FETCH_ASSOC], [], ["projekte.id" => $this->projekt_id], [], ["version" => true]);
        if (!empty($res)){
        ...
        // get auslagen liste
        $res = $this->db->dbFetchAll('auslagen', [DBConnector::FETCH_ASSOC], ['auslagen.id', 'auslagen.name_suffix', 'auslagen.state', 'auslagen.created'], ["auslagen.projekt_id" => $this->projekt_id], [], ['auslagen.id' => true]);
        if (!empty($res)){
            $aus = [];
            foreach ($res as $row){
                $aus[] = $row;
    ...
    private function getDbProjektPosten(){
        $res = $this->db->dbFetchAll("projektposten", [DBConnector::FETCH_ASSOC], [], ["projekt_id" => $this->projekt_id]);
        $aus = [];
        if (!empty($res)){
            foreach ($res as $row){
                $aus[] = $row;
    ...
    public function state_change($newState, $etag): bool{
        ...
        if ($this->state_change_possible($newState)){
            ...
            if ($newState == 'ok-belege' || $newState == 'ok-hv' || $newState == 'ok-kv'){
                $tmp_auslage = $this->db->dbFetchAll('auslagen', [DBConnector::FETCH_ASSOC], [], ['id' => $this->auslagen_data['id']]);
                if (
                    $tmp_auslage

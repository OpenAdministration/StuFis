...
class HibiscusXMLRPCConnector
	 ...
	 */
	public function updateKontoIDs(){
		...
		try{
			...
			foreach ($ktos as $kto){
				...
				if (!isset($dbKtos[$ktoId])){
					...
					$msgs[] = "<strong>Konto $ktoId:</strong> $ktoName ($short) (IBAN: $ktoIBAN) wurde neu gefunden" .
						(($ret_tmp !== false) ? " und hinzugefügt" : " konnte aber nicht hinzugefügt werden!");
					$ret = $ret && ($ret_tmp !== false);
				}
			...
			$deletedInHibiscus = array_diff(
				...
				[0]
			); // 0 ist reserviert für die Handkasse
			foreach ($deletedInHibiscus as $id){
				...
				if ($affectedRows > 0){
					$msgs[] = "<strong>Konto $id:</strong> {$dbKto["name"]} (IBAN: {$dbKto["iban"]})" .
						"kann im FINTS nicht mehr gefunden werden. Die Syncronisation wird eingestellt.";
				}
			}
	...
	/**
	 * @return array|int returns umsätze or Error Code
	 */
	public function fetchAllUmsatz(){
		list($success, $msgs) = $this->updateKontoIDs();
		if (!$success){
			return [false, $msgs, []];
		}
		// get data from RPC
		try{
			...
			foreach ($this->fetchableKontos as $ktoid => $kto){
				...
				$umsatz = array_merge($umsatz, $client->list($umsopt));
				usort(
					$umsatz,
					function($a, $b){
						if ($a["id"] < $b["id"])

	...
	class MenuRenderer
                                                ...
                                                <div class="panel panel-default">
                                                    ...
                                                    <div class="panel-heading collapsed <?= (!isset($auslagen[$id]) || count(
                                                        ...
                                                        <h4 class="panel-title">
                                                            ...
                                                            <span class="label label-info project-state-label"><?= ProjektHandler::getStateString(
																	$projekt["state"]
																) ?></span>
                                                        </h4>
                                                    </div>
																	...
																	$this->renderTable(
																		...
																		[
																			...
																			function($stateString){
																				$text = AuslagenHandler2::getStateString(
		...
		private function renderBooking($active){
						...
						while (isset($alZahlung[$idxZahlung]) && floatval($alZahlung[$idxZahlung]["value"]) === $value){
							...
							switch ($alZahlung[$idxZahlung]['konto_id']){
								case 1: //girokonto
									...
									$caption .= $alZahlung[$idxZahlung]["empf_name"] . " - " . explode(
											"DATUM",
											$alZahlung[$idxZahlung]["zweck"]
										)[0];
									break;
								case 2: //kasse
            ...
            <form action="<?= URIBASE ?>rest/booking" method="POST" role="form" class="ajax-form">
                <div class="booking__panel-form col-xs-2">
                    ...
                    <input type="hidden" name="nonce" value="<?= $GLOBALS["nonce"] ?>">
                    <button type="submit" id="booking__check-button" class="btn btn-primary">
                        Buchung anweisen
                    </button>
                </div>
            </form>
		...
		private function renderKontoRefreshButton(){ ?>
            <form action="<?= URIBASE ?>rest/hibiscus" method="POST" role="form"
                  class="form-inline ajax d-inline-block">
		...
		private function renderBookingText(){
			...
			foreach ($allInstructions as $vorgang_id => $instruction){
				...
				$zahlungenDB = DBConnector::getInstance()->dbFetchAll(
					"konto",
					[DBConnector::FETCH_ASSOC],
					[],
					["id" => ["IN", $zahlungen]]
		...
		private function saveBooking(){
			...
			if (count($bookingDBbelege) + count($bookingDBzahlung) > 0){
				ErrorHandler::_renderErrorPage(
					[
						"msg" => "Beleg oder Zahlung bereits verknüpft - " . print_r(
			...
			if ($stateChangeOk !== true){
				...
				ErrorHandler::_renderErrorPage(
					[
						"msg" => "Beleg kann nicht in Status 'gebucht' überführt werden - evtl. wurde noch nicht auf gezahlt gesetzt? ",
						"code" => "500 Interner Fehler"
					]
				);
			}
			...
			if ($diff >= 0.01){
				DBConnector::getInstance()->dbRollBack();
				ErrorHandler::_errorExit(
					"Falsche Daten wurden übvertragen: Differenz der Posten = $diff (" . var_export(
						$diff >= 0.01,
						true
					) . ")"
				);

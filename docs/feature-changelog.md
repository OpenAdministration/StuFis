...
* Sicherheit im Bankimport (FinTS): Die Formulare der Bankzugang-Seiten sind nun gegen Anfragen von fremden Seiten abgesichert (CSRF-Schutz). Bisher wurde das mitgesendete Sicherheitsmerkmal auf diesen Seiten nicht geprüft – eine fremde Webseite konnte dadurch im Namen einer angemeldeten kassenverantwortlichen Person Login-Versuche auslösen (drei Fehlversuche sperren den Bankzugang bei der Bank), Zugangsdaten anlegen oder ein beliebiges Konto zur Synchronisation eintragen. Ist ein Formular nicht mehr gültig, wird die Anfrage nun mit einem Hinweis abgelehnt.
* Bankimport (FinTS): Ein Konto aus dem Bankzugang wird nun über die normale Seite „Konto anlegen“ eingerichtet – mit schon eingetragener IBAN, und nach dem Speichern geht es zurück zur Kontenübersicht des Bankzugangs. Damit gelten dort dieselben Prüfungen wie überall sonst: gültige IBAN, eindeutiges Kürzel, Pflichtangaben und ein gültiges Startdatum. Das eigene, ungeprüfte Formular im Bankzugang entfällt; zuvor konnte darüber ein Konto mit ungültiger IBAN oder doppeltem Kürzel entstehen, dessen Umsätze dann gar nicht oder im falschen Konto landeten.
* Bankimport (FinTS): Bei einem Konto, das aus einem Bankzugang übernommen wird, sind die IBAN und der Schalter „Manuelles Eintragen möglich“ nun gesperrt – mit Begründung, statt sie auszublenden. Manuelles Eintragen würde die automatische Synchronisation ausschließen, die für dieses Konto ja gerade eingerichtet wird.
* Bankimport (FinTS): Der Aufruf des Umsatzimports für ein noch nicht eingerichtetes Konto führte zu einer Fehlerseite; nun wird auf das Anlegen des Kontos hingewiesen. Gehört ein Konto nicht zum gewählten Bankzugang, wird das ebenfalls verständlich gemeldet.
* Bankimport (FinTS): Bei Konten ohne hinterlegtes Startdatum wird der Zeitraum nun der Bank überlassen, statt ein Datum zu erfinden. Zuvor führte ein fehlendes Startdatum zum Abbruch.

...
* Nachtragshaushaltsplan: Zu einem genehmigten oder aktiven Haushaltsplan kann jetzt ein Nachtrag erstellt werden, der Titel ändert, hinzufügt oder streicht, ohne den laufenden Plan direkt zu verändern. Der Nachtrag durchläuft denselben Freigabe-Workflow wie ein Haushaltsplan und wird erst mit seiner Aktivierung (sofort oder zu einem geplanten Aktivierungsdatum) im Hauptplan wirksam; bereits gebuchte Titel bleiben dabei unter derselben ID erhalten. Ein Nachtrag kann optional eine eigene Bezeichnung erhalten (z. B. für Sitzungsvorlagen); ohne Angabe wird ersatzweise das Erstellungsdatum angezeigt.
* Haushaltstitel: In der Detailansicht eines Titels ist jetzt zu sehen, ob und durch welche Nachträge er geändert wird – jeweils mit Art der Änderung, Status des Nachtrags und der hinterlegten Begründung. Da mehrere Nachträge parallel in Arbeit sein dürfen, werden alle betroffenen gleichzeitig aufgeführt.
* Haushaltsplan: Beim Wechsel in einen weiter fortgeschrittenen Status werden die Titel nun geprüft. Eine doppelt vergebene Titelnummer, ein leerer Titelname oder ein negativer Betrag verhindern den Statuswechsel und werden benannt, statt erst später Folgefehler zu verursachen. Bei einem Nachtrag wird die Titelnummer zusätzlich gegen den Ursprungsplan geprüft, damit keine Dopplung entsteht, die erst beim Wirksamwerden auffällt. Schritte zurück bleiben immer möglich.
* Haushaltsplan: Beschluss-, Genehmigungs- und Aktivierungsdatum werden nun direkt im Dialog des Statuswechsels erfasst – also in dem Moment, in dem das jeweilige Datum entsteht – statt losgelöst davon im Bearbeiten-Formular. Die Angabe bleibt optional, und die Daten sind in der Plan-Ansicht weiterhin sichtbar.
